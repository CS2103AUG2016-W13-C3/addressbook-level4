...
``` html
                ...
                <p>
                    ...
                    <blockquote>
                        Note that for the <em>Model</em> component, besides the API defined by <code>Model.java</code>, it also contains data structures such as <code>UserPrefs.java</code> and <code>ReadOnlyToDo.java</code> that the other components are dependent on. Such dependencies are listed in the later section <a href="#model-component">Model Component.</a>
                    </blockquote>

                <p>Underlying all 4 components is the package <code>seedu.commando.commons</code>. It is a collection of utility classes and application constants that are used in all the components. We also use <a href="#eventscenter">Event Driven Design</a> for components to communicate with one another.</p>

                <h3>What Happens in a Command Execution</h3>
                ...
                <p>
                    ...
                    <ul>
                        ...
                        <li>Relaying the data of <em>Model</em> to the <em>UI</em> through the <code>getUiEvents()</code>, <code>getUiTasks()</code> and <code>getToDoList()</code></li>
                    </ul> 
                </p>

                <p>The <code>Logic</code> interface has the following methods:</p>
                
                <div class="datagrid class">
                    <table>
                        <thead>
                            <tr>
                                <th>&lt;&lt;interface&gt;&gt;<br />Logic</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr><td>+ execute(commandText: String): CommandResult</td></tr>
                            <tr><td>+ getUiEvents(): UnmodifiableObservableList&lt;UiToDo&gt;</td></tr>
                            <tr><td>+ getUiTasks(): UnmodifiableObservableList&lt;UiToDo&gt;</td></tr>
                             <tr><td>+ getToDoList(): ReadOnlyToDoList</td></tr>
                        </tbody>
                    </table>
                </div>

                <h3>When <code>execute(String)</code> is Called</h3>
                <p>
                ...
                <p>The <code>Model</code> interface has the following methods:</p>

                <div class="datagrid class">
                    <table>
                        <thead>
                            <tr>
                                <th>&lt;&lt;interface&gt;&gt;<br />Model</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr><td>+ getUiEvents(): UnmodifiableObservableList&lt;UiToDo&gt;</td></tr>
                            <tr><td>+ getUiTasks(): UnmodifiableObservableList&lt;UiToDo&gt;</td></tr>
                            <tr><td>+ getToDoList(): ReadOnlyToDoList</td></tr>
                            <tr><td>+ getUiToDoAtIndex(index: int): Optional&lt;UiToDo&gt;</td></tr>
                            <tr><td>+ changeToDoList(change: ToDoListChange)</td></tr>
                            <tr><td>+ undoToDoList()</td></tr>
                            <tr><td>+ redoToDoList()</td></tr>
                            <tr><td>+ clearUiToDoListFilter(ifHistoryMode: boolean)</td></tr>
                            <tr><td>+ setUiToDoListFilter(keywords: Set&lt;String&gt;, tags: Set&lt;Tags&gt;, ifHistoryMode: boolean)</td></tr>
                        </tbody>
                    </table>
                </div>
                ...
                <p>
                    The <em>Storage</em> component (package <code>seedu.commando.storage</code>) is in charge of the reading of the data for the to-do list and the data for user preferences on the file system, as well as the saving of this data to the file system.
                </p>

                <p>The <code>Storage</code> interface has the following methods:</p>

                <div class="datagrid class">
                    <table>
                        <thead>
                            <tr>
                                <th>&lt;&lt;interface&gt;&gt;<br />Storage</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr><td>+ setToDoListFilePath(path: String)</td></tr>
                            <tr><td>+ readToDoList(): Optional&lt;ReadOnlyToDoList&gt;</td></tr>
                            <tr><td>+ readToDoList(path: String): Optional&lt;ReadOnlyToDoList&gt;</td></tr>
                            <tr><td>+ saveToDoList(toDoList: ReadOnlyToDoList)</td></tr>
                            <tr><td>+ saveToDoList(toDoList: ReadOnlyToDoList, path: String)</td></tr>
                            <tr><td>+ readUserPrefs(): Optional&lt;UserPrefs&gt;</td></tr>
                            <tr><td>+ saveUserPrefs(userPrefs: UserPrefs)</td></tr>
                            <tr><td>+ setUserPrefsFilePath(path: String)</td></tr>
                        </tbody>
                    </table>
                </div>
                ...
                <p>
                    For components to inform one another that something has happened, we use <a href="https://github.com/google/guava/wiki/EventBusExplained">Google's Event Bus library</a>, packaged in the <code>EventsCenter</code> class within <code>seedu.commando.commons</code>. This is especially important for lower-level components to communicate with higher-level components that they should not be dependent on. An example is the posting of <code>ExitAppRequestEvent</code> by <em>Logic</em> to inform <code>MainApp.java</code> to close the window.
                </p>
