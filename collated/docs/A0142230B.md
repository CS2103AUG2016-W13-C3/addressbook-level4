...
``` html
                ...
                <p>There are situations where the time window of an Event or the due date of a <em>Task</em> become no longer relevant, including the recurrence (If there is one). Similarly, use the <code>delete</code> command, ending with the keyword <code>time</code>. Note that this means that a target <em>Event</em> will become a <em>Task</em> after such a command.</p>
                ...
                <h4>Deleting Recurrence of Events </h4>

                <p>If the recurrence of an <em>Event</em> is no longer required, you'll also use the <code>delete</code> command, ending with the keyword <code>recurrence</code>.</p>

                <span><strong>Command Formats:</strong></span>
                <pre class="command-format"><span class="inputstart">&gt;&gt;</span> delete <span>&lt;index&gt;</span>... recurrence</pre>
                <pre class="command-format"><span class="inputstart">&gt;&gt;</span> delete <span>&lt;start index&gt;</span> - <span>&lt;end index&gt;</span> recurrrence</pre>
                ...
                <p>*You can delete multiple fields by chaining keywords.</p>
